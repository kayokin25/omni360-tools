...
function match(targets, creatives) {
  ...
  for (const t of targets.values()) {
    ...
    let best = null;
    if (isRuss(t.op)) {
      best = byDim.get(w + 'x' + h) || null;      // строго точный размер
      t.rule = 'точное совпадение (Russ Outdoor)';
    } else {
      const fit = reps.filter(function (c) {
        const cr = L.reduceRatio(c.dims[0], c.dims[1]);
        return cr[0] === tr[0] && cr[1] === tr[1] && c.dims[0] >= w && c.dims[1] >= h;
      });
      if (fit.length) {
        best = fit.reduce(function (a, b) {
          const aa = a.dims[0] * a.dims[1], bb = b.dims[0] * b.dims[1];
          if (aa !== bb) return aa < bb ? a : b;
          return a.dims[0] <= b.dims[0] ? a : b;
        });
      }
      t.rule = 'тот же AR, размер ≥ экрана';
    }
    t.match = best;
    if (best) {
...
function buildReport(targets, unknown, creatives, skippedArchives) {
  ...
  const s1 = {
    name: 'Отправка',
    cols: [22, 13, 8, 9, 34, 13, 22, 26],
    rows: [['Оператор', 'Размер экрана', 'AR', 'Кол-во ФО', 'Файл креатива',
            'Размер видео', 'Масштабирование', 'Длительность']]
  };
  for (const t of used) {
    const cw = t.match.dims[0], ch = t.match.dims[1];
    const scale = t.exact ? 'точно' : 'вниз (' + cw + 'x' + ch + ' → ' + t.w + 'x' + t.h + ')';
    s1.rows.push([
      t.op, t.w + 'x' + t.h, t.ar, t.count,
      { v: t.match.name, s: L.S.OK },
      cw + 'x' + ch, scale, summarizeDurations(t.durs)
    ]);
  }
  ...
  const usedDims = new Set(used.map(function (t) { return t.match.dims[0] + 'x' + t.match.dims[1]; }));
  const s3 = {
    name: 'Лишние видео',
    cols: [34, 13, 8, 60],
  ...
  for (const c of sorted) {
    ...
    if (c.dims) {
      ...
      const key = c.dims[0] + 'x' + c.dims[1];
      s3.rows.push([c.name, key, rr[0] + ':' + rr[1],
        usedDims.has(key)
          ? 'дубль по размеру — этот размер уже закрыт другим файлом'
          : 'не подошёл ни к одной поверхности (нет такого размера/AR в АП или есть крео ближе по размеру)']);
    } else {
