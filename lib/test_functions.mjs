/* Проверка функций-прокси без деплоя: вызываем обработчики напрямую.
   Запуск:  GIS_API_KEY=<ключ> node lib/test_functions.mjs

   Лежит в lib/, а НЕ в netlify/functions/: всё, что попадает в папку функций,
   Netlify пытается собрать и задеплоить, а обработчика здесь нет — сборка
   сайта падала бы. */
import gis from '../netlify/functions/gis.mjs';
