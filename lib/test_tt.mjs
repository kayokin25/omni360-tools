...
console.log('=== длительность из АП главнее справочника ===');
const apTable = {
  sheetName: 'Все', headerRow: 1,
  colOf: { 'Оператор': 0, 'Формат экрана': 1, 'Разрешение': 2, 'Длительность, с': 3 },
  rows: [
    ['РИМ', 'BILLBOARD', '720x360', 7],             // в справочнике 5 — берём 7 из АП
    ['РИМ', 'BILLBOARD', '720x360', 12],            // та же связка, другая длительность
    ['РИМ', 'BILLBOARD', '720x360', '7 с'],         // повтор не размножает
    ['Russ Outdoor', 'CITY_FORMAT', '640x880', ''], // пусто → справочник
    ['НетТакого', 'BILLBOARD', '100x100', '']       // ни там, ни там
  ]
};
const ap = T.processAp(apTable, null);
const apDedup = T.deduplicate(ap.results);
eq('строк после дедупа', apDedup.length, 3);
eq('несколько длительностей через запятую', apDedup[0]['Длительность (сек)'], '7, 12');
eq('пусто в АП → справочник', apDedup[1]['Длительность (сек)'], 10);
eq('нет ни в АП, ни в справочнике', apDedup[2]['Длительность (сек)'], '');
eq('в промахи попала только связка без справочника', [...ap.durationMisses], ['НетТакого | BILLBOARD']);
eq('одна длительность остаётся числом', T.joinDurations(T.parseDurations('10 сек')), 10);
