...
/* 1. шапка отчёта едет вправо вместе с листом: столбец A теперь узкий, под «№»,
      и прижатые к нему длинные подписи обрезались бы о соседнее значение */
assert.match(out, /<c r="B1"[^>]*>[\s\S]*?Рекламодатель/, 'шапка отчёта сдвигается вместе с листом');
assert.match(out, /<c r="C1"[^>]*>[\s\S]*?А7 — форумы/, 'значение шапки — следом за подписью');
assert.ok(!/<c r="A1"/.test(out), 'столбец «№» в шапке отчёта пустой');
