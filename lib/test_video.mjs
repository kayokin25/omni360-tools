...
console.log('=== buildArgs: картинка в видео ===');
eq('картинка 10 с, 1440x720 — один кадр крутится, звука нет',
  V.buildArgs({ inName: 'pic.jpg', outName: 'out.mp4', still: true, targetDur: 10,
                scale: { w: 1440, h: 720 } }),
  ['-loop', '1', '-i', 'pic.jpg', '-t', '10', '-vf', 'scale=1440:720', '-r', '25',
   '-c:v', 'libx264', '-preset', 'veryfast', '-pix_fmt', 'yuv420p', '-crf', '20', '-an',
   '-movflags', '+faststart', '-y', 'out.mp4']);

eq('без длительности картинка крутится десять секунд, а не бесконечно',
  V.buildArgs({ inName: 'pic.png', outName: 'out.mp4', still: true }),
  ['-loop', '1', '-i', 'pic.png', '-t', '10', '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2', '-r', '25',
   '-c:v', 'libx264', '-preset', 'veryfast', '-pix_fmt', 'yuv420p', '-crf', '20', '-an',
   '-movflags', '+faststart', '-y', 'out.mp4']);

eq('нечётная картинка округляется до чётных сторон — libx264 иначе падает',
  V.buildArgs({ inName: 'pic.png', outName: 'out.mp4', still: true, targetDur: 5 })
    .filter(function (x, i, arr) { return arr[i - 1] === '-vf'; }),
  ['scale=trunc(iw/2)*2:trunc(ih/2)*2']);

eq('картинке не нужен -stream_loop, даже если задать исходную длительность',
  V.buildArgs({ inName: 'pic.jpg', outName: 'out.mp4', still: true, srcDur: 1, targetDur: 5 })
    .slice(0, 6),
  ['-loop', '1', '-i', 'pic.jpg', '-t', '5']);
