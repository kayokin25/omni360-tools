...
console.log('\n--- Отправка ---');
for (const rowOrGroup of rep.sheets[0].rows) {
  const r = Array.isArray(rowOrGroup) ? rowOrGroup : (rowOrGroup.cells || []);
  const indent = (!Array.isArray(rowOrGroup) && rowOrGroup.outlineLevel === 1) ? '   ↳ ' : '   ';
  console.log(indent + r.map(c => (c && typeof c === 'object' ? c.v : c)).join(' | '));
}
console.log('\n--- Не хватает ---');
...
// keep=3, а не 5: с правилом «один, самый большой файл на соотношение
// сторон» (см. match() и match_creatives.py) те же 8 связок закрываются
// тремя файлами вместо пяти — 600x300 и 540x270 (2:1) теперь покрывает
// файл 720x360 той же группы, а не свои отдельные.
console.log('\n=== СВЕРКА С ЭТАЛОНОМ (Отчет_подбор_крео.xlsx) ===');
const want = { targets: 12, used: 8, missing: 4, keep: 3, creatives: 108, unknown: 0 };
let ok = true;
